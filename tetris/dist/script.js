...
class Board {
  constructor() {
    ...
    this.init();
    this.score = 0;
    this.highScore = 0; // Menyimpan skor tertinggi
  }

  spawnNextShape() {
    const shapeTypes = [Line, Square, LShape, ZShape, TShape];
    const ShapeClass = shapeTypes[this.getRandomRange(0, shapeTypes.length - 1)];
    this.nextShape = new ShapeClass(0, 4);
    this.renderNextShape();
  }

  renderNextShape() {
    const nextShapeContainer = $(".next-shape");
    nextShapeContainer.empty(); // Menghapus konten sebelumnya
    for (let block of this.nextShape.getBlocks()) {
        const blockElement = $("<div class='block'></div>");
        blockElement.css({
            width: "28px",
            height: "28px",
            position: "absolute",
            left: block.y * 28 + "px", // Menggunakan ukuran balok
            top: block.x * 28 + "px" // Menggunakan ukuran balok
        });
        nextShapeContainer.append(blockElement);
    }
  }
  ...
  init() {
    // Inisialisasi elemen dan nilai awal
    $("#score").text(" " + this.score);
  ...
  gameUpdate() {
    if (this.isGameOver()) {
      ...
      if (this.score > this.highScore) {
        ...
        $("#high-score").text("Higt Score: " + this.highScore); // Tampilkan skor tertinggi
    }
    }
  }
